...
class Planet:
    # Астрономическая еденица
    AU = 149.6e6 * 1000
    # Гравитационная постоянная
    G = 6.67428e-11
    SCALE = 250 / AU  # 1ае в в 100 пикселей
    TIMESTEP = (3600*24) / SPEED_SCALE  # 1 день в сенкунуд
    ...
    def draw(self, win):
        # для начала отображения в центре
    ...
    def attraction(self, other):
        # вычисление силы притяжения между двумя объектами
        ...
        # вычислить угол треугольника и узнать расстояние между двумя объектами

        ugol = math.atan2(distance_y, distance_x)
        force_x = math.cos(ugol) * force
        force_y = math.sin(ugol) * force
        return force_x, force_y

    def update_position(self, planets):
        total_fx = total_fy = 0
        for planet in planets:
            # силу притяжения к другим планетам
            if self == planet:
                continue
        ...
        # Рассчитать ускорени по формуле  a = F / m
...
def main():
    ...
    while run:
        clock.tick(20)
        # экран обновляется при каждом повторение
        WIN.fill((0, 0, 0))
